...
<Note>
  Workflow endpoints with no `api.eachlabs.ai` equivalent (get/update workflow, categories, public workflows) are served from `https://workflows.eachlabs.run/api/v1`, a fully supported host for those endpoints.
</Note>
