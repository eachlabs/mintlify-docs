...
| `depends_on` | string[] | Public pricing dimensions, such as resolution or token type; may be empty |
| `rows` | object[] | Price breakdown as `{label, value}` entries in USD per `unit`; may be empty |
