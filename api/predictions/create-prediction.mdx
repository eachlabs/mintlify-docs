...
description: "Create a new prediction for a specific AI model."
---
...
After creating a prediction, the model processes your input asynchronously. You have two options:

1. **Poll** — Use [Get Prediction](/api/predictions/get-prediction) to check status
2. **Webhook** — Provide a `webhook_url` and receive results automatically when ready

<Note>
  Model input parameters vary. Use [Get Model](/api/models/get-model) to retrieve the `request_schema` for any model.
</Note>
