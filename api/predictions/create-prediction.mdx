...
| `webhook_secret` | string | No | Secret used to sign webhook requests. When set, each delivery carries `X-Webhook-Signature` and `X-Webhook-Timestamp` — see [Verifying Webhook Signatures](/api/webhooks/verifying-signatures) |
...
<Note>
  Every model has different input parameters. Use [Get Model](/api/models/get-model) to grab the `request_schema` for any model and see exactly what it expects.
</Note>
