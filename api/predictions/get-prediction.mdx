...
if data["status"] == "success":
    ...
    print(f"Failed: {data.get('output')}")
```
...
if (result.status === "success") {
  ...
  console.log(`Failed: ${JSON.stringify(result.output)}`);
}
```
</CodeGroup>
...
| `output` | string \| array \| object | Model-specific prediction result or failure detail. V1 does not guarantee one failure shape |
...
V1 does not define a structured failure contract inside `output`. Most models
return a customer-readable string, while some legacy integrations preserve an
object or array. Treat the value as model-specific display/debug information;
do not parse its wording or branch on undocumented object fields.
...
See [Error Reference](/errors) for retry guidance. Log the prediction ID when a
