...
For prediction failures, branch on the machine-readable `output.error_code`.
Display `output.error_message` to users, but do not parse its wording or depend
on provider-specific text.
...
See [Error Reference](/errors) for common codes and retry guidance. Some legacy
or unclassified failures can still use an older output shape; do not derive an
error code by parsing free-form text.
