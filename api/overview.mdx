...
description: "Access 600+ AI models through one simple, unified API."
---
...
| `GET` | `/v1/executions` | List prediction executions and costs |
...
The model catalog (`GET /v1/models`, `GET /v1/models/{slug}`, `GET /v1/llm-router/model-catalog`) is public; all other endpoints require an API key via the `Authorization: Bearer` header.
