...
Permanently deletes a file you uploaded with [Upload File](/api/uploads/presign-upload). Pass the `id` returned by the presign step. A successful delete removes the stored object, so the file's `public_url` stops resolving.
...
|-----------|------|----------|-------------|
| `id` | string (UUID) | Yes | The file id returned by [Upload File](/api/uploads/presign-upload) — the `id` field of the presign response. |
