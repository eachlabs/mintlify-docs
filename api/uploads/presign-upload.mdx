...
```json
{
  ...
  "expires_at": "2026-10-24T18:15:00Z",
  "required_headers": {
...
| `expires_at` | string | ISO-8601 timestamp when the **stored object** expires and is deleted (default 180-day retention). This is **not** the upload-URL deadline — the `presigned_url` itself is only valid for 15 minutes after issuance (see [Limits](#limits)). |
...
| Presigned URL lifetime | **15 minutes** | How long `presigned_url` accepts the `PUT`, measured from issuance. Not returned in the response; request a new presign if it lapses. |
| Stored object retention | **180 days** (default) | How long the uploaded file is kept before deletion. This is the value reported by `expires_at`. |
...
<Note>
  The presigned URL expires **15 minutes** after it's issued — this is separate from `expires_at`, which reports the stored object's retention (default 180 days), not the upload deadline. Upload soon after requesting; if the URL lapses, just request a new one.
</Note>
