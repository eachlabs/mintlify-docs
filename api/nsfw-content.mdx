...
<Warning>
  This list might not cover everything! Model support for this parameter can change as we add or update models. Check the model's input schema via `GET /v1/models/{slug}` to confirm whether `enable_safety_checker` is accepted.
</Warning>
