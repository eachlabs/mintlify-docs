...
- **A repeated body is expected.** A redelivery sends a body you have already seen, with a current timestamp and its own valid signature. Verify it exactly as you would a first delivery — the tolerance check is about when the delivery was signed, not about whether the body is new. Nothing is loosened by accepting it: a delivery signed long ago still carries that old timestamp and still falls outside your tolerance.
- **Do not use the signature to detect duplicates.** Each delivery of the same body is signed separately, so their signatures will not match. Use `execution_id` from the body to recognise work you have already done.
