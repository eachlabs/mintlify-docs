...
Deliveries are attempted up to 3 times in total: the initial attempt plus 2 retries, spaced roughly 10 seconds apart. Every attempt of a delivery carries the same signature and timestamp. You can track delivery attempts using the [Get Webhook](/api/webhooks/get-webhook) endpoint.
