...
# Trigger a workflow (returns 202 with execution_id and status)
response = requests.post(
    f"{BASE}/workflows/trigger/WORKFLOW_ID/v1",
    headers=HEADERS,
    json={
        "inputs": {
...
# Poll for completion
while True:
    r = requests.get(f"{BASE}/workflows/executions/{execution_id}", headers=HEADERS)
    status = r.json()
