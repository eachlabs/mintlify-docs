...
each::sense is OpenAI-compatible, so you can use the official OpenAI Python SDK:
...
For full control over event types, use `requests` directly:
...
For direct model access via each::api:
