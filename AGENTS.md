...
- **video/mcp/tools.mdx is generated** by the private video-api-mcp repo's `scripts/generate-tool-docs.mjs` — never hand-edit it; regenerate and re-commit. A drift-check job mirroring video-api-mirror is a TODO in `docs.yml` (blocked on a video-api-mcp read token and the generator shipping in the npm package).
