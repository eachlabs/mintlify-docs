...
Run an AI model with specified parameters. **Terminal** — returns result directly.
...
Start execution of a workflow. **Non-terminal** — agent monitors execution.
...
Ask the user for more information. **Terminal** — yields a `clarification_needed` event and waits.
