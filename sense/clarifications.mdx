...
- **`agent` mode** — Agent makes assumptions when possible, asks only when truly ambiguous
- **`ask` mode** — Agent always asks before executing
- **`plan` mode** — Agent shows plan and asks for approval
