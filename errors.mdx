...
{
  "status": "error",
  "output": "This model is temporarily rate limited. Retry shortly."
}
```

In V1, prediction failure `output` is model-specific. Most models return a
customer-readable string, while some legacy integrations preserve an object or
array. V1 does not guarantee a machine-readable error code in this field. Do
not parse message wording or depend on undocumented object keys.
...
above. When an accepted prediction later fails, display its `output` as failure
detail and log the prediction ID. Retry transient failures with bounded
exponential backoff; correct invalid input before retrying. Contact support with
the prediction ID when the cause is unclear or persists.
