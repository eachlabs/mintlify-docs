...
There is no per-second request limit on predictions. A 429 means an **account concurrency cap**: while your balance is **$10.00 or less**, at most **10 predictions may be in flight at once** — **2** while the price of the requested model is not yet known. A rejected request creates no prediction and is never billed. Read `details` for the cap that was hit.
