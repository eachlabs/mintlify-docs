...
<Note>
Video generation is async. Processing can take 30 seconds to several minutes depending on model and duration. Use [webhooks](/api/webhooks/overview) for production workflows. Pricing varies, so check the [dashboard](https://eachlabs.ai) for the latest.
</Note>
