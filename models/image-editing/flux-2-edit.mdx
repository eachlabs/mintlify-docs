...
- Provide clear, specific editing instructions in the prompt
- Works best with high-quality source images
- Can handle color changes, object modifications, style transfers, and more
- For region-specific edits, consider using `flux-fill-pro` with a mask instead
...
<Note>
Call `GET /v1/model?slug=flux-2-edit` for the full parameter schema.
</Note>
