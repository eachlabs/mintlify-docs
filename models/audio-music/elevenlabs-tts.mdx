...
- Lower `stability` values produce more expressive, dynamic speech
- Higher `similarity_boost` keeps the voice closer to the reference
- Supports multiple languages with the multilingual model
- Max duration is approximately 10 minutes of audio per request
- Use SSML-like punctuation (commas, periods, dashes) to control pacing
...
<CardGroup cols={2}>
  ...
  <Card title="Stable Audio 2.5" href="/models/audio-music/stable-audio-2-5">
    Sound effects and short audio generation.
  </Card>
</CardGroup>

<Note>
Call `GET /v1/model?slug=elevenlabs-tts` for the full parameter schema.
</Note>
