...
<Note>
This is a curated subset. For the full model list, call `GET /v1/models` or browse the [dashboard](https://www.eachlabs.ai).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Video Generation" icon="video" href="/models/video-generation">
    Text-to-video and image-to-video models. Cinematic, fast, and high-quality.
  </Card>
  <Card title="Image Editing" icon="wand-magic-sparkles" href="/models/image-editing">
    Edit, fill, upscale, remove backgrounds, and swap faces in existing images.
  </Card>
  <Card title="Audio & Music" icon="music" href="/models/audio-music">
...
<Steps>
  <Step title="Find the model slug">
    Browse this catalog or call `GET /v1/models` to find the model slug you need.
  </Step>
  <Step title="Check input parameters">
    Each model card lists required and optional parameters. You can also call `GET /v1/models/{slug}` for the full schema.
  </Step>
