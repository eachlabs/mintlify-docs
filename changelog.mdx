...
<Update label="September 5, 2026" description="API" rss={{ title: "Transcribe audio with Meta Muse Voice", description: "API" }}>
  ## Transcribe audio with Meta Muse Voice

  - `meta-muse-voice-transcribe` is available for audio transcription requests through `POST /v1/audio/transcriptions`, so you can send audio files to Meta Muse Voice from the same API surface as other transcription models.
</Update>

<Update label="September 4, 2026" description="Legal" rss={{ title: "Review the latest Terms, DPA, AUP, and Privacy Policy", description: "Legal" }}>
  ## Review the latest Terms, DPA, AUP, and Privacy Policy

  - [Terms of Service](https://eachlabs.ai/terms-of-service) now publishes version 2.0 with its effective date and archived version link, so you can review the active terms and the version they replaced.
  - [Data Processing Addendum](https://eachlabs.ai/dpa) is available as a public page with versioned archive support, so the DPA referenced by the Terms is reachable directly.
  - Sign-in and sign-up now name and link the Data Processing Addendum alongside the Terms of Service and Privacy Policy, so online acceptance points to the full agreement set.
  - The Acceptable Use Policy is published, and the Privacy Policy has been updated to align with the Terms of Service and Data Processing Addendum.
</Update>

<Update label="September 4, 2026" description="Billing" rss={{ title: "Identify the organization behind each spending alert", description: "Billing" }}>
  ## Identify the organization behind each spending alert

  - Spending alert emails now include the organization name, so users who belong to multiple organizations can tell which account triggered the alert without cross-checking IDs.
</Update>

<Update label="September 3, 2026" description="AI Models" rss={{ title: "Use Qwen3.8 Flash and GLM 5.3 Flash", description: "AI Models" }}>
  ## Use Qwen3.8 Flash and GLM 5.3 Flash

  - `qwen/qwen3.8-flash` is available in the model catalog and routing layer, so you can select Qwen3.8 Flash for LLM requests.
  - `z-ai/glm-5.3-flash` is available in the model catalog and routing layer, so you can select GLM 5.3 Flash for LLM requests.
</Update>
