...
<Update label="August 29, 2026" description="Creative Studio" rss={{ title: "Build Creative Studio projects with clearer flow and real assets", description: "Creative Studio" }}>
  ## Build Creative Studio projects with clearer flow and real assets

  - Select multiple asset files at once in Creative Studio; each file gets a pending preview tile and uploads in order, so adding a batch no longer means repeating the picker one file at a time.
  - The Creative Studio home, mode selection, and first Movie Studio step now make the Video vs Movie choice, required project details, and next action clearer, so you can start a project without guessing what is missing.
  - The project browser is paged and searchable beyond the first loaded set of projects, so larger libraries are easier to scan.
  - Movie Studio now uses real storyboard and asset generation flows instead of fixture content, with shared project assets across the studio experience so generated work stays connected to the project.
  - Creative Studio generation feedback and cost details now use one clearer output pattern, so you can see what is running, what it produced, and what it cost in one consistent place.
</Update>

<Update label="August 27, 2026" description="each::workflows" rss={{ title: "Keep workflow media with the workflow", description: "each::workflows" }}>
  ## Keep workflow media with the workflow

  - Media uploaded while building workflow definitions can be stored without an expiration, so files used in step parameters, fallback parameters, and input defaults stay tied to the workflow.
  - Uploads can request non-expiring storage with `expires_in_seconds=-1`, so workflow-level media can follow the workflow's lifecycle.
</Update>
