...
<Update label="August 19, 2026" description="API" rss={{ title: "List executions across all organization API keys", description: "API" }}>
  ...
  - `GET /v1/executions` can now return organization-wide execution history for
    cost and usage reporting from a single authenticated API key.
  - Omit `api_key_ids` to keep the previous behavior and list only executions
    created with the authenticated key.
  - Send `api_key_ids=` with an empty value to include all API keys in the
    organization, or pass comma-separated API key IDs to narrow the result.
</Update>
