...
<Update label="September 25, 2026" description="Webhooks" rss={{ title: "Redeliver a webhook from the dashboard", description: "Webhooks" }}>
  ...
  - Signed webhooks are signed again when the redelivery is sent, so it passes your timestamp tolerance rather than looking like a replayed request. See [Redeliveries are signed again](/api/webhooks/verifying-signatures#redeliveries-are-signed-again).
