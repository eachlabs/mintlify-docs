...
<Update label="July 27, 2026" description="LLM Router" rss={{ title: "Preview: OpenAI-compatible audio transcription and speech", description: "LLM Router" }}>
  ## Preview: transcribe audio files and stream speech

  - The LLM Router documentation now covers two OpenAI-compatible audio
    endpoints: `POST /v1/audio/transcriptions` for bounded file transcription
    and `POST /v1/audio/speech` for progressively streamed text-to-speech.
  - Both use the same API key and base URL as chat completions, so the OpenAI
    SDK works with a base URL change alone.
  - These endpoints are in **staged preview** and are enabled per environment.
    A `404` means the capability is not enabled for your deployment yet — talk
    to [support@eachlabs.ai](mailto:support@eachlabs.ai) before depending on
    them in production.
</Update>
