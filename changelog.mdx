...
<Update label="September 21, 2026" description="Workflows" rss={{ title: "Focus on error evidence in workflow history", description: "Workflows" }}>
  ## Focus on error evidence in workflow history

  - **Behavior change:** Workflow execution history no longer shows trace analysis or generated “How to fix” suggestions in preview or classic views. Use the retained error messages, structured metadata, field errors, and raw error details to investigate failures without misleading diagnostic advice.
</Update>

<Update label="September 21, 2026" description="API" rss={{ title: "Ask structured questions with the Decisions API", description: "API" }}>
  ...
  - The model is routed through OpenRouter to TypeSafe Jev. This API is in **alpha** and its contract may change — see the [Decisions API](/llm-router/decisions) page.
</Update>

<Update label="September 21, 2026" description="API" rss={{ title: "Webhook deliveries are now signed with HMAC-SHA256", description: "API" }}>
  ...
  - The new [Verifying Webhook Signatures](/api/webhooks/verifying-signatures) guide covers the procedure, with Python and JavaScript handlers.
