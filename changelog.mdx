...
<Update label="August 19, 2026" description="API" rss={{ title: "List executions across all organization API keys", description: "API" }}>
  ...
  - **Behavior change:** this endpoint previously returned only the executions
    created with the authenticated key. If you rely on that narrower result,
    pass that key's ID in `api_key_ids`.
  - `api_key_ids=123,456` narrows the result to specific keys in your
    organization; omit it to include them all. Key IDs come from the API keys
    page in the dashboard, and IDs outside your organization match nothing.
  - Scope never widens past your own organization, and the response shape is
    unchanged.
</Update>
