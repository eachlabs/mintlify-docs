...
- Long jobs: the server polls with backoff; if a job outlives the polling budget — the hosted server holds up to ~4 minutes, the local stdio server defaults to 5 (configurable via `EACHLABS_POLL_TIMEOUT_MS`) — the tool returns the prediction ID and your agent resumes with `get_job`.
