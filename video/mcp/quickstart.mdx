...
The agent chains the `trim` and `captions` tools; each tool call returns the hosted artifact URL (or JSON for the [analysis capabilities](/video/capabilities#response-shapes)) plus the billed cost. Local files: [upload them to eachlabs storage](/storage/upload-file) first and give the agent the returned `public_url`.
