...
One tool per wrapped capability — every capability in the [capabilities reference](/video/capabilities) is exposed as a tool, under the reference's name except for the `keep_ranges`/`silence_remove` pair, which keeps its original wire name, `silence_remove` — plus `run_ffmpeg` (raw ffmpeg for beta organizations; contract and safety boundary in [run_ffmpeg](/video/capabilities#run_ffmpeg)) and `get_job` for checking a long-running job later. A few tools accept a subset of the REST parameters, so each tool's own schema is the authority on what it takes. Parameters are validated client-side before submit and again by the engine. The MCP tool layer drops any parameter a tool's schema does not declare, so it never reaches the engine — an over-specified call still submits and bills, silently running without that parameter. (Sent straight to the REST API the same unknown key is rejected by the engine at $0, but the tool layer strips it first.)
