...
Every capability job is submitted the same way: `POST https://api.eachlabs.ai/v1/prediction` with header `X-API-Key`, body `{"model": "eachlabs-video-api", "input": {"capability": "<name>", "tier": "cpu", "input_url": "...", "params": {...}}}`, then poll `GET /v1/prediction/{id}` until a terminal status. `tier` is **required** (`cpu` or `gpu`). Analysis capabilities (`probe`, `scene_detect`, `silence_detect`) return structured JSON in `output`; everything else returns hosted artifact URL(s) — the exact JSON per output class is in [Response shapes](#response-shapes). See [Quickstart](/video/quickstart) for the input URL requirements and [Billing & Limits](/video/billing-limits) for pricing mechanics and the error catalog. For long-tail cases the named capabilities don't cover, a raw-ffmpeg mode with a different top-level contract is documented at [run_ffmpeg](#run_ffmpeg), and multi-step patterns are collected under [Recipes](#recipes).

The 37 capabilities by category (sections below are alphabetical):

| Category | Capabilities |
|-|-|
| Transform & geometry | `crop`, `pad`, `reframe`, `reframe_track`, `rotate`, `scale` |
| Encode & convert | `gif`, `transcode` |
| Time & speed | `fade`, `reverse`, `segment`, `silence_remove`, `speed`, `trim` |
| Audio | `audio_replace`, `extract_audio`, `loudnorm`, `volume` |
...
| Analysis | `probe`, `scene_detect`, `silence_detect` |
...
Re-encoding capabilities that expose no codec knobs (`crop`, `scale`, `pad`, `rotate`, `overlay`, `watermark`, `speed`, `reverse`, `fade`, `color`, `reframe`, `reframe_track`, `concat`, `captions`, `lut3d`, `silence_remove`, `slideshow`) share one fixed encode contract, keyed on the `container` parameter:
...
- CRF, audio bitrate, and sample rate are left at ffmpeg's defaults (effectively CRF 23 for x264 and 128k AAC) unless the capability exposes a `crf` knob (`transcode`, `poster`, `segment` with `reencode: true`).
- The gpu tier pins `-c:v h264_nvenc -pix_fmt yuv420p`.
- Deviations: `volume` and `loudnorm` stream-copy the video (mp4/mkv only) and touch only audio; `slideshow` encodes at fixed quality 23; `concat` resamples audio to 44.1 kHz stereo.
...
<Warning>
Chunks produced by stream-copy `trim` carry discard-flagged pre-roll back to the previous keyframe, which becomes **visible** after `concat_copy` (measured +1.35s on a 30s target) — use `silence_remove` (its `ranges` parameter) for frame-accurate multi-range editing.
</Warning>
...
- loudnorm outputs mp4/mkv video containers only. For audio-in/audio-out mastering (e.g. MP3 to a -14 LUFS MP3) use the [run_ffmpeg](#run_ffmpeg) mastering example.
</Note>
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · `gpu` · **Output:** media — URL array in `output`, one entry per chunk, index-ordered (`seg_000`, `seg_001`, …) — see [Response shapes](#response-shapes)
...
Cut a clip of roughly `duration` seconds via stream copy — no re-encode. The cut snaps to the keyframe at or before `start` (playback start is trimmed via an edit list); the end can drift by up to packet granularity. For frame-accurate cuts use `silence_remove` with a single `ranges` entry.
...
<Warning>
Stream-copy trim outputs carry discard-flagged pre-roll back to the previous keyframe — players skip it, but it becomes **visible** after `concat_copy` (measured +1.35s on a 30s target). Use `silence_remove` (its `ranges` parameter) for frame-accurate multi-range editing.
</Warning>
...
Every result arrives in the polled prediction's `output` field. There are exactly five shapes, determined by the capability's output class; the sibling fields (`status`, `metrics`) are identical across all of them.
...
**2. Multi-output URL array** — `segment`, `thumbnails`. Same array, one URL per produced file, index-ordered (`seg_000`, `seg_001`, …):
...
**5. Analysis JSON** — `probe`, `scene_detect`, `silence_detect` (and `run_ffmpeg` with `ffprobe`). `output` is the JSON document itself; no artifact is uploaded:
...
<Note>
Stream-copy `trim` carries +0.00-0.17s of end drift per segment. When exact boundaries matter, use `silence_remove` (its `ranges` parameter; re-encodes) instead of per-segment trims.
</Note>
