...
<Note>
**Beta.** The Video API is in private beta: these pages are shared by direct link and are not listed in the public navigation. The surface below is validated server-side with `additionalProperties: false` — a request is either exactly this shape or it is rejected before any compute runs, at $0. Contracts here are stable for the beta (additions may land; nothing is removed without notice).
</Note>

<Note>
Using these from an AI agent? The [MCP server](/video/mcp/quickstart) exposes most capabilities as native tools — its page lists the current tool surface.
</Note>
