...
4. **Tier and encoder constraints** — webm-on-gpu, cpu-only codecs, and gpu output ceilings computed from your params are checked at submit; ceiling checks that need the source's own dimensions (e.g. `keep_ranges`, `segment` with `reencode`) run at execute after the probe and are skipped when the source cannot be probed. Each is noted per capability.
