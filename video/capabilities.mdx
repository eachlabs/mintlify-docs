...
<Note>
**Exact-shape validation.** The surface below is validated server-side with `additionalProperties: false` — a request is either exactly this shape or it is rejected before any compute runs, at $0. Contracts here are stable; additions may land, removals will not without notice.
</Note>
...
- The script is **not parsed or validated server-side**: a malformed sidecar fails at execution as a metered `exec_error` (settles $0), not a pre-run reject. Validate it locally before submitting.
...
Raw ffmpeg/ffprobe for long-tail cases the named capabilities don't cover. This section is the canonical statement of the `run_ffmpeg` contract — other pages link here.
