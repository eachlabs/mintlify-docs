...
<Note>
**Private beta.** These pages are shared by direct link only and are not listed in the public navigation. The surface below is validated server-side with `additionalProperties: false` — a request is either exactly this shape or it is rejected before any compute runs, at $0. Contracts here are stable for the beta; additions may land, removals will not without notice.
</Note>

Every capability job is submitted the same way: `POST https://api.eachlabs.ai/v1/prediction` with header `Authorization: Bearer YOUR_API_KEY`, body `{"model": "eachlabs-video-api", "input": {"capability": "<name>", "tier": "cpu", "input_url": "...", "params": {...}}}`, then poll `GET /v1/prediction/{id}` until a terminal status. `tier` is **required** and its only accepted value is `"cpu"`. Analysis capabilities (`probe`, `scene_detect`, `silence_detect`, `audio_analysis`) return structured JSON in `output`; everything else returns hosted artifact URL(s) — the exact JSON per output class is in [Response shapes](#response-shapes). See [Quickstart](/video/quickstart) for the input URL requirements and [Billing & Limits](/video/billing-limits) for pricing mechanics and the error catalog. For long-tail cases the named capabilities don't cover, a raw-ffmpeg mode with a different top-level contract is documented at [run_ffmpeg](#run_ffmpeg), and multi-step patterns are collected under [Recipes](#recipes).
...
<Warning>
`cpu` is the only tier available. A `gpu` tier is not currently offered — a request carrying `tier: "gpu"` is rejected at $0 before any compute runs. Every capability below runs on `cpu`.
</Warning>
...
4. **Tier and encoder constraints** — `cpu` is the only accepted tier, so a `tier: "gpu"` request is rejected at $0 before the gate runs. Encoder constraints that hold on `cpu` — a codec the requested container cannot mux, such as `segment`/`silence_split` with `reencode: true` targeting `webm` — are checked at submit. Each is noted per capability.
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** analysis JSON — `{"audio_analysis": {...}}` in `output`; no uploaded artifact, zero egress
...
**Inputs:** `input_urls` (2) — `input_urls[0]` is the music/program bed, `input_urls[1]` is the voice · **Tiers:** `cpu` · **Output:** media — single audio file (mp3/wav/m4a)
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** media — single audio file (mp3/wav/m4a)
...
**Inputs:** `input_urls` (2) — `input_urls[0]` is the base video, `input_urls[1]` is the audio source · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_urls` (2) — `input_urls[0]` is the video, `input_urls[1]` is the subtitle file (SRT/VTT/ASS) · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_urls` (2-10, must equal the `inputs` param; joined in array order) · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_urls` (2-50, must equal the `inputs` param; joined in array order) · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_urls` (2) — joined in order, first into second · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** media — single output.gif
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** bundle `{"primary": master.m3u8, "artifacts": [...]}` in `output` — master.m3u8 + stream_N.m3u8 variants + shared TS segments — see [Response shapes](#response-shapes)
...
**Inputs:** `input_urls` (2) — `input_urls[0]` is the video, `input_urls[1]` is the `.cube` LUT file · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_urls` (2) — `input_urls[0]` is the base video, `input_urls[1]` is the overlay (video or image) · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** bundle `{"primary": master.m3u8, "artifacts": [...]}` in `output` — manifest.mpd + master.m3u8 + fmp4 segments — see [Response shapes](#response-shapes)
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** bundle `{"primary": master.m3u8, "artifacts": [...]}` in `output` — master.m3u8 + manifest.mpd over shared fmp4 segments — see [Response shapes](#response-shapes)
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** named-outputs object `{"video": ..., "thumbnail": ...}` in `output` — see [Response shapes](#response-shapes)
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** analysis JSON — the ffprobe document (`format` + `streams`) returned directly in `output`; no uploaded artifact
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** media — single file at the preset resolution
...
**Inputs:** `input_urls` (2) — `input_urls[0]` is the source video, `input_urls[1]` is the crop-track sidecar (sendcmd script) · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** analysis JSON — `{"scenes": [{time, score}]}` in `output` (`time` in seconds, `score` in 0-1); no uploaded artifact, zero egress
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** media — URL array in `output`, one entry per chunk, index-ordered (`seg_000`, `seg_001`, …); with `first_frame: true` the stills are interleaved with their clips (clip, still, clip, still, … in index order) — see [Response shapes](#response-shapes)
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** analysis JSON — `{"silences": [{start, end, duration}]}` in `output` (all fields in seconds); no artifact, zero egress
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** media — URL array in `output`, one entry per segment, index-ordered (`seg_000`, `seg_001`, …) — see [Response shapes](#response-shapes)
...
**Inputs:** `input_urls` (2-50, must equal the `images` param; shown in array order) · **Tiers:** `cpu` · **Output:** media — single silent video
...
**Inputs:** `input_urls` (2) — `input_urls[0]` is the base video, `input_urls[1]` is the sticker (image or video) · **Tiers:** `cpu` · **Output:** media — single file
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** media — single image
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** named-outputs object `{"sheet": ..., "vtt": ...}` in `output` — see [Response shapes](#response-shapes)
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** media — URL array in `output`, one entry per frame, index-ordered (`thumb_001`, `thumb_002`, …) — see [Response shapes](#response-shapes)
...
**Inputs:** `input_url` (1) · **Tiers:** `cpu` · **Output:** media — single file, container-chosen extension
...
**Inputs:** `input_urls` (2) — `input_urls[0]` is the base video, `input_urls[1]` is the watermark (image or video) · **Tiers:** `cpu` · **Output:** media — single file
