...
- **Retry the same request** only for `platform_fault`, the one transient failure class — identify it by `error_message`, because the envelope's `retryable` field reads `false` for it.
- **Resubmit after backoff** for `PROVIDER_RATE_LIMIT`, which the envelope's `retryable` field does mark `true`. Throttling is load-dependent, not a request defect — a later resubmit can succeed.
- **Everything else not marked `retryable: true`** — including `platform_drift` — is deterministic and will fail again: fix the request or the input instead of retrying.
