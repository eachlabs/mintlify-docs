...
<Note>
**Private beta.** These pages are shared by direct link only and are not listed in the public navigation. Rates below are the current beta rate card and may be adjusted during the beta with notice; the *mechanics* (integer seconds, no multipliers, $0 rejects) are the contract.
</Note>
...
`cpu` is the only value `tier` accepts. A `gpu` tier is not currently offered — a request carrying `tier: "gpu"` is rejected at $0 before any compute runs.
...
| Case | Example |
|-|-|
| Invalid request | Unknown capability, wrong enum, out-of-range value, unknown parameter key, a missing or non-`cpu` `tier`, wrong input count |
...
| `encoder_unavailable` | 400 | Requested encoder not on the tier's image | No |
| `filter_unavailable` | 400 | Requested filter not on the tier's image | No |
| `output_resolution_too_large` | 400 | Output dimension over the tier's limit | No |
...
| `platform_fault` | `PROVIDER_FAILED`, `error_message: "platform_fault"` | $0 | Platform | **Yes** — retry the same request. Identify it by `error_message`, not the envelope `retryable` field, which reads `false` even here. |
...
| `tier` | Required on every request, value `cpu` | A missing, empty, or `gpu` tier is rejected at $0 |
