...
- **A low-balance concurrency cap is the one per-organization limit.** While the organization balance is **$10.00 or less**, at most **10 predictions may be in flight at once** — **2** while the price of the requested model is not yet known. A submit past the cap answers HTTP `429` with the cap named in `details`; it creates no prediction and is never billed. Keeping the balance above $10.00 removes the cap entirely. See the [Error Reference](/errors#429-too-many-requests).
