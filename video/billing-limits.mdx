...
| Parameters that would produce an empty output — when the source can be probed | `trim` starting at/past the end of the clip, `fade` scheduled beyond the video's duration, sub-frame output windows. Unsupported input formats are rejected; on other probe failures the gate fails open and the job runs and bills its execution seconds |
...
| ffmpeg fails mid-run after N seconds, on input content that passed the pre-compute probe gate (corrupt media, failed subtitle conversion, an impossible mux, an out-of-bounds crop/pad window) | Nothing | `ceil(N)` seconds, 5 s floor applies | `exec_error` | No |
...
- **No per-organization request-rate limit on submits today** (beta). `POST /v1/prediction` is not throttled per key or per org by requests per second.
- **A low-balance concurrency cap is the one per-organization limit.** While the organization balance is **$10.00 or less**, submits are capped by in-flight count: **2 concurrent jobs** for a model the platform cannot price before it runs — video jobs bill from metered seconds, so they sit here — against 10 for a model with a fixed list price. A submit past the cap answers HTTP `429` with the cap named in `details`; it creates no prediction and is never billed. Keeping the balance above $10.00 removes the cap entirely. See the [Error Reference](/errors#429-too-many-requests).
