...
| `retryable` | Derived from `error_code`, not from the engine's failure class: `true` for the throttling, engine-5xx, and transport codes, `false` for everything else — including a post-accept `platform_fault`, which is transient but still reads `false`. Not sufficient on its own; see [Retries](#retries--read-before-automating) |
