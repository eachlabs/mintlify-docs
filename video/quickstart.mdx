...
**Input playlists are unsupported.** HLS, DASH, and caller-supplied concat playlists cannot be used as inputs, including a playlist renamed with a supported extension. Supply the underlying media files separately; use [concat](/video/capabilities#concat) or [concat_copy](/video/capabilities#concat_copy) to join them. HLS/DASH **output** remains supported by the packaging capabilities and raw `run_ffmpeg` mode. [Captions](/video/capabilities#captions) continues to accept SRT, VTT, and ASS subtitle files with style overrides.
