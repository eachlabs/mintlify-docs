...
`input_url` (and `input_urls`) accept a **publicly downloadable `https` URL** to a media file — we stage your media into eachlabs storage automatically before processing. Private footage: a presigned URL from your own storage works — any `https` URL the engine can fetch; media is downloaded into eachlabs storage for processing (see [Acceptable Use](/video/acceptable-use)). `s3://` input URIs from your onboarding keep working unchanged, and you can chain jobs by passing a previous job's `output` URL as the next job's input.
