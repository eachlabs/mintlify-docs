...
<Note>
**Beta.** The Video API is in private beta: these pages are shared by direct link and are not listed in the public navigation. The API is production infrastructure — request shapes documented here are stable for the beta (additions may land; nothing is removed without notice). Billing is live from the first call: see [Billing & Limits](/video/billing-limits).
</Note>
