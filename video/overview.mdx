...
- **Strict validation, $0 rejects.** Every capability validates its parameters server-side with `additionalProperties: false`. Invalid parameters — wrong enum, out-of-range value, unknown key — are rejected before execution and cost nothing. An input failure during metered execution, including subtitle conversion, follows the failed-compute rules below.
...
- **Empty outputs are rejected at $0 when the source can be probed.** Requests whose parameters would produce nothing (a trim starting past the end of the clip, a fade beyond the video's duration) are rejected by a probe gate before execution. Unsupported input formats are also rejected. On other probe failures, the empty-output gate fails open: the job runs and bills its execution seconds even if the output is near-empty.
