...
<Note>
**Private beta.** These pages are shared by direct link only and are not listed in the public navigation. The API is production infrastructure: request shapes documented here are stable for the beta — additions may land, removals will not without notice.
</Note>
...
- **Per-second integer billing, no multipliers.** You pay `billed_seconds × tier rate`, plus output delivery only beyond a 200 MB free allowance per job ($0.30/GB on the excess). No resolution, codec, or duration multipliers. Details in [Billing & Limits](/video/billing-limits).
