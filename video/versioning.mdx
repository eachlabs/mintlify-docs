...
- Request shape: `capability`, `tier`, `input_url` / `input_urls`, `params`, `output_format`.
...
- **The tier set.** Only `cpu` is offered today; a request for any other tier is rejected at $0. Offering another tier later is a compatible change.
