...
- **Versions** — Each version contains a complete definition
...
Each step defines an operation in the workflow. Steps can be model invocations, HTTP requests, Python functions, parallel branches, conditional logic, or pass-through operations.
...
All branches start simultaneously and execute independently. The parallel step completes when all branches finish.

---

## Choice Steps (Conditional Logic)

Use `type: "choice"` to route execution based on a condition. The workflow evaluates the condition and executes either the `condition_met_branch` or the `default_branch`.
...
A pass-through step that does no processing. Useful for injecting static values or as a placeholder.
...
Steps execute sequentially by default. Each step's output is available to subsequent steps via [parameter references](/workflows/concepts/parameter-references). Parallel and choice steps enable branching and conditional logic within the flow.
...
When a fallback is triggered, a separate step output appears with a `_fallback` suffix:
...
Fallback reasons: `primary_failed` (primary step failed, fallback executed) or `not_triggered` (primary succeeded, fallback skipped).
