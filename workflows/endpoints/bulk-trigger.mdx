...
Run the same workflow with up to **10 different inputs** in parallel. All executions share a `bulk_id` for tracking.
...
Some executions may succeed while others fail. Failed entries have `status: "failed"` with an error `message` and no `execution_id`.
