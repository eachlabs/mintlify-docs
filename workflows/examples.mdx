...
# 2. Trigger execution (202 with execution_id and status)
execution = requests.post(
    f"{BASE_URL}/workflows/trigger/{workflow_id}/{version_id}",
    headers=HEADERS,
...
while True:
    result = requests.get(
        f"{BASE_URL}/workflows/executions/{execution_id}",
        headers=HEADERS
    ).json()
