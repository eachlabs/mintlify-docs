...
A two-step workflow that enhances a prompt with an LLM, then generates an image.
...
Generate a portrait image, animate it, and upscale the result.
...
Run image-to-video, image editing, and text-to-image concurrently, then conditionally upscale based on output format.
...
1. **Parallel step** — Three branches run concurrently: video generation, image editing, and text-to-image
2. **Choice step** — Checks if the primary output is a video (`.mp4`) and routes to the appropriate upscaler

## Logical Conditions — AND/OR Operators

Use logical operators to combine multiple conditions in a choice step.

This example only upscales when the output is a PNG **and** the user requested upscaling:
