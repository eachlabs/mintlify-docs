...
When the trigger included a `webhook_secret`, the delivery also carries:

| Header | Meaning |
|--------|---------|
| `X-Webhook-Signature` | `sha256=` plus a lowercase hex HMAC-SHA256 over `<X-Webhook-Timestamp>.<raw request body>` |
| `X-Webhook-Timestamp` | Unix time in seconds at which the delivery was signed |

Verify the signature over the **raw** request body — see [Verifying Webhook Signatures](/api/webhooks/verifying-signatures).
...
- Set a `webhook_secret` and [verify the signature](/api/webhooks/verifying-signatures) before trusting a payload
