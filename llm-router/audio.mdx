...
| Text-to-speech | `POST /v1/audio/speech` | `deepgram/aura-2`, `google/gemini-3.1-flash-tts-preview` | Progressive raw MP3 or PCM bytes |
...
| `google/gemini-3.1-flash-tts-preview` | `Zephyr`, `Puck`, `Kore`, `Charon`, `Aoede` | `pcm` | 5,000 characters |

`google/gemini-3.1-flash-tts-preview` returns `pcm` only — requesting `mp3`
for it is rejected with a `400`. It does not accept `speed` or `instructions`.
