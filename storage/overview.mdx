...
Files are stored on AWS infrastructure in the **United States** and delivered through the `cdn-us.eachlabs.ai` CDN over HTTPS.

## Data retention

Uploaded files are retained for **180 days**, after which they're automatically removed.

To remove a file sooner, call [Delete File](/storage/delete-file). Deletes are permanent and idempotent — once a file is gone, its `public_url` stops resolving.
